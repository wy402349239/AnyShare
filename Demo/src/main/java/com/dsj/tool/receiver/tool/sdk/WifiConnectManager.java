package com.dsj.tool.receiver.tool.sdk;
...
public class WifiConnectManager extends WifiManagerWrap {
    ...
    private void handleNetworkStateChanged(NetworkInfo networkInfo) {
        if (networkInfo.isConnected() && mWifiConnectListener != null) {
            String ssid = NetworkUtils.getCurrentSSID(mContext);
            if (ssid != null) {
                if (ssid.contains(SSID_PREFIX)) // 2014-1-21 add
                    mWifiConnectListener.onWifiConnected();
            } else {
                Toast.makeText(mContext, "您当前WiFi存在异常，请重新连接", Toast.LENGTH_SHORT).show();
            }
        } else if (!networkInfo.isConnected() && mWifiDisconnectListener != null) { //2014-1-22 add
